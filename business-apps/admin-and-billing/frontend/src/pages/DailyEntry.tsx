...
const DailyEntry = () => {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const res = await axios.get(
        `/api/daily-entry?date=${date()}${userId ? `&user_id=${userId}` : ""}`,
      );
      setLogs(res.data || []);
    } catch (error) {
  ...
  const handleDelete = async (logId: number) => {
    ...
    if (
      !confirm(
        "Are you sure you want to delete this entry? This will refund the cost.",
      )
    )
      return;
    ...
    <div class="space-y-6 animate-in">
      ...
      <form onSubmit={handleSubmit} class="space-y-4">
            ...
            <ul class="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden">
              <For each={suggestions()}>
                {(user) => (
                  ...
                  >
                    <div>
                      <span class="font-bold text-lg text-slate-900 block">
                        {user.name}
                      </span>
                    ...
                    <span
                      class={`font-bold text-lg ${user.balance < 0 ? "text-red-600" : "text-green-600"}`}
                    >
        ...
        <div class="grid grid-cols-3 gap-3 pt-2">
          <label
            class={`card p-3 flex flex-col items-center justify-center gap-2 cursor-pointer transition-all border-2 ${mealCategory() === "standard" ? "border-blue-500 bg-blue-50" : "border-transparent"}`}
          >
            <input
              type="radio"
              name="cat"
            ...
            />
            <span
              class={`font-bold text-sm ${mealCategory() === "standard" ? "text-blue-900" : "text-slate-600"}`}
            >
              {t("standard")}
            </span>
          </label>
          <label
            class={`card p-3 flex flex-col items-center justify-center gap-2 cursor-pointer transition-all border-2 ${mealCategory() === "special" ? "border-purple-500 bg-purple-50" : "border-transparent"}`}
          >
            <input
              type="radio"
              name="cat"
            ...
            />
            <span
              class={`font-bold text-sm ${mealCategory() === "special" ? "text-purple-900" : "text-slate-600"}`}
            >
              Special
            </span>
          </label>
          <label
            class={`card p-3 flex flex-col items-center justify-center gap-2 cursor-pointer transition-all border-2 ${mealCategory() === "none" ? "border-orange-500 bg-orange-50" : "border-transparent"}`}
          >
            <input
              type="radio"
              name="cat"
            ...
            <Salad
              size={32}
              class={
                mealCategory() === "none" ? "text-orange-600" : "text-slate-400"
              }
            />
            <span
              class={`font-bold text-sm text-center ${mealCategory() === "none" ? "text-orange-900" : "text-slate-600"}`}
            >
              {t("extrasOnly")}
            </span>
          </label>
        ...
        <div class="pt-2 pb-24">
          ...
          <div class="grid grid-cols-2 gap-3">
            <ExtraItem
              label={t("rice")}
              price={prices()["rice"] ?? 10}
              value={extraRice()}
              onChange={setExtraRice}
            />
            <ExtraItem
              label={t("roti")}
              price={prices()["roti"] ?? 4}
              value={extraRoti()}
              onChange={setExtraRoti}
            />
            <ExtraItem
              label={t("chicken")}
              price={prices()["chicken"] ?? 30}
              value={extraChicken()}
              onChange={setExtraChicken}
            />
            <ExtraItem
              label={t("fish")}
              price={prices()["fish"] ?? 20}
              value={extraFish()}
              onChange={setExtraFish}
            />
            <ExtraItem
              label={t("egg")}
              price={prices()["egg"] ?? 10}
              value={extraEgg()}
              onChange={setExtraEgg}
            />
            <ExtraItem
              label={t("vegetable")}
              price={prices()["vegetable"] ?? 15}
              value={extraVegetable()}
              onChange={setExtraVegetable}
            />
          </div>
        </div>
      ...
      <Portal>
        ...
        <div
          class={`fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-green-600 text-white px-6 py-3 rounded-full shadow-xl flex items-center gap-2 transition-all duration-300 pointer-events-none ${successMsg() ? "translate-y-0 opacity-100" : "-translate-y-20 opacity-0"}`}
        >
          <Check size={20} />
    ...
    <div class="space-y-6">
      <div class="bg-slate-200 p-1 rounded-xl flex">
        <button
          type="button"
          onClick={() => setMealType("lunch")}
          class={`flex-1 py-3 text-sm font-bold rounded-lg ${mealType() === "lunch" ? "bg-white shadow-sm text-amber-600" : "text-slate-600"}`}
        >
          Lunch
        </button>
        <button
          type="button"
          onClick={() => setMealType("dinner")}
          class={`flex-1 py-3 text-sm font-bold rounded-lg ${mealType() === "dinner" ? "bg-white shadow-sm text-indigo-600" : "text-slate-600"}`}
        >
          Dinner
        </button>
      </div>
...
const ExtraItem = (props: {
  ...
  <div class="card p-3 flex flex-col items-center bg-white shadow-sm border border-slate-200">
    ...
    {props.price > 0 && (
      <span class="text-xs text-slate-400 mb-2">₹{props.price}</span>
    )}
    <div class="flex items-center gap-3 mt-1 w-full justify-center">
      <button
        type="button"
        onClick={() => props.onChange(Math.max(0, props.value - 1))}
        class="w-12 h-12 rounded-full bg-slate-100 active:bg-slate-300 text-slate-700 font-bold text-2xl flex items-center justify-center"
      >
        -
      </button>
      <span class="text-2xl font-black text-slate-900 w-6 text-center">
        {props.value}
      </span>
      <button
        type="button"
        onClick={() => props.onChange(props.value + 1)}
        class="w-12 h-12 rounded-full bg-blue-100 active:bg-blue-300 text-blue-700 font-bold text-2xl flex items-center justify-center"
      >
        +
      </button>
    </div>
