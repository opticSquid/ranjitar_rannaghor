  ...
  const generateReport = async () => {
    ...
    try {
      const res = await axios.get(
        `/api/reports/bill?user_id=${selectedUser()}&start_date=${startDate()}&end_date=${endDate()}`,
      );
      setReport(res.data);
    } catch (err) {
    ...
    <div class="space-y-6 pb-24 animate-in">
            ...
            {showSuggestions() && (
              <ul class="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden">
                ...
                {suggestions().length === 0 && (
                  <li class="p-6 text-center text-slate-500 font-medium">
                    No customers found
                  </li>
                )}
              </ul>
      ...
      <Show when={report()}>
        <div class="animate-in slide-in-from-bottom">
          ...
          <div
            id="bill-content"
            class="bg-white text-slate-800 p-6 sm:p-12 rounded-xl shadow-md max-w-4xl mx-auto border border-slate-200"
          >
            {/* Bill Header */}
            ...
            <div class="grid grid-cols-2 gap-6 mb-8 text-sm">
              <div>
                <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                  {t("billTo")}
                </h4>
                ...
                <p class="text-slate-600 font-medium">
                  {report()?.user.mobile_no}
                </p>
                <p class="text-slate-600 font-medium">
                  Room {report()?.user.room_no}
                </p>
              </div>
              <div class="text-right">
                <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                ...
                <div class="space-y-1 text-sm font-medium">
                  <p class="text-slate-500">
                    {t("openingBalance")}{" "}
                    <span class="text-slate-800 font-bold ml-2">
                  ...
                  <p class="text-slate-500">
                    {t("recharges")}{" "}
                    <span class="text-slate-800 font-bold ml-2">
                      ₹{report()?.total_recharges.toFixed(2)}
                    </span>
                  </p>
                </div>
              </div>
              ...
              <table class="w-full mb-8 text-sm">
                <thead>
                  <tr class="bg-slate-100 text-slate-600 text-left">
                    <th class="py-2 px-3 font-bold text-[10px] uppercase tracking-wider">
                      {t("date")}
                    </th>
                    <th class="py-2 px-3 font-bold text-[10px] uppercase tracking-wider">
                      {t("meal")}
                    ...
                    <th class="py-2 px-3 font-bold text-[10px] uppercase tracking-wider text-right">
                      {t("amount")}
                    </th>
                  </tr>
                </thead>
                  ...
                  <For each={report()?.logs}>
                    {(log) => (
                      <tr class="text-slate-800">
                        <td class="py-3 px-3">
                          {formatLocalDate(log.log_date)}
                        </td>
                        <td class="py-3 px-3 font-bold capitalize text-slate-600">
                          {log.meal_type}
                        </td>
                        <td class="py-3 px-3">
                          {log.has_main_meal ? (
                        ...
                        <td class="py-3 px-3 font-black text-right">
            ...
            <div class="flex justify-end pt-4 border-t-2 border-slate-200">
              <div class="w-64 space-y-3">
                ...
                <div
                  class={`p-3 rounded-xl mt-3 flex justify-between items-center ${report()!.closing_balance < 0 ? "bg-red-50 text-red-700 border border-red-200" : "bg-green-50 text-green-700 border border-green-200"}`}
                >
