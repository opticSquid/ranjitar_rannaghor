    ...
    return (
        <div class="space-y-6 pb-24 animate-in">
            <header>
                ...
                <p class="text-slate-500 font-medium text-sm mt-1">{t('manageCosts')}</p>
            </header>
            ...
            <div class="card bg-red-50 border-red-200 flex flex-col items-center justify-center p-6">
                <span class="text-sm font-bold text-red-700 uppercase tracking-wider mb-1">{t('total')} Expenses</span>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-bold text-slate-500 mb-1 pl-1 uppercase tracking-wider">{t('startDate')}</label>
                    <input
                        type="date"
                        value={startDate()}
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 mb-1 pl-1 uppercase tracking-wider">{t('endDate')}</label>
                    <input
                        type="date"
                        value={endDate()}
                    ...
                    {(expense) => (
                        <div class="card p-4 flex flex-col gap-3 bg-white border border-slate-200">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h4 class="font-bold text-lg text-slate-900">{expense.reason}</h4>
                ...
                <div class="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm flex items-end sm:items-center justify-center">
                    <div class="bg-white w-full max-w-[600px] rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl pb-safe animate-in slide-in-from-bottom max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleSubmit} class="space-y-4">
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-1">{t('amount')}</label>
                                <div class="relative">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-1">{t('reasonNote')}</label>
                                <textarea
                                    required
                            ...
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-1">{t('date')}</label>
                                <input
                                    type="date"
                                    required
                                    value={formDate()}
                            ...
                            >
                                {isSubmitting() ? 'Saving...' : (editingExpense() ? t('saveChanges') : t('addExpense'))}
                            </button>
                        </form>
                    </div>
                </div>
